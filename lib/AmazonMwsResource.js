...
AmazonMwsResource.prototype = {
  ...
  _responseHandler: function(
    ...
    function parseCSVFile(res, responseString, delimiter, callback) {
      ...
      csv
        .fromString(responseString, options)
    ...
    function processResponseType(res, responseString, callback) {
      //debug('res %o ', res);
      var xmlParser = new xml2js.Parser(AmazonMwsResource.configXml2JS);
      if (!res.headers["content-type"]) {
        ...
        RESPONSE_CONTENT_TYPE_XML.indexOf(
        ...
        var TAB_DELIMITER = "\t";
        var COMMA_DELIMITER = ",";
        parseCSVFile(res, responseString, TAB_DELIMITER, function(
          error,
          response
        ) {
          if (!_.isEmpty(error)) {
    ...
    return function(res) {
      ...
      if (
        ...
        if (
          contentType.split(";")[1] &&
          contentType.split(";")[1].match(/^((\b[^\s=]+)=(([^=]|\\=)+))*$/)[3]
        ) {
          charset = contentType
            .split(";")[1]
            .match(/^((\b[^\s=]+)=(([^=]|\\=)+))*$/)[3];
      ...
      var ResponseHeaders = {
        ...
        "x-mws-quota-remaining":
          res.headers["x-mws-quota-remaining"] || "unknown",
        "x-mws-quota-resetson":
          res.headers["x-mws-quota-resetson"] || "unknown",
          ...
          try {
            ...
            return callback.call(
              ...
              new Error.AmazonMwsAPIError({
                message:
                  "Failed to parse response received from the AmazonMws API",
        ...
        try {
          ...
          processResponseType(res, responseString, function(error, response) {
            ...
            if (RESPONSE_CONTENT_TYPE_XML.indexOf(contentType) > -1) {
              ...
              if (
                response[requestParamsJSONCopy.Action + "Result"] !== undefined
              ) {
  ...
  _request: function(method, path, data, auth, options, callback) {
    ...
    if (self.requestParams.Action === "SubmitFeed") {
      ...
      self.requestParams.ContentMD5Value = crypto
        .createHash("md5")
        .update(self.body)
        .digest("base64");
    ...
    var sorted = _.reduce(
      _.keys(self.requestParams).sort(),
      function(m, k) {
    ...
    var stringToSign = [
      method,
      self._AmazonMws.getApiField("host"),
      path,
      qs.stringify(sorted)
    ].join("\n");
    ...
    self.requestParams.Signature = crypto
      .createHmac("sha256", this._AmazonMws.getApiField("secret"))
    ...
    var requestParamsJSONCopy = JSON.parse(
      JSON.stringify(self.requestParamsJSON)
    );
    ...
    function makeRequest(requestParamsCopy) {
      ...
      var isInsecureConnection =
        self._AmazonMws.getApiField("protocol") === "http";
